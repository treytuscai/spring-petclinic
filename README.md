...
The tutorial above describes how to configure Jenkins, SonarQube, credentials, webhooks, and the pipeline job manually through multiple browser-based steps. This replaces all of that with two files: a Jenkins Configuration as Code file (`casc.yaml`) that declaratively defines Jenkins state at boot, and a shell script (`setup.sh`) that orchestrates the full stack in one command.
